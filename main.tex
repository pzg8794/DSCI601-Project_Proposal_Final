...
% \vspace{0.25em}
\caption{\modified{Clinical routing illustration under incomplete versus
quality context. The left panel shows the harm mechanism: weak-context routing
over-allocates testing to lower-burden majority-White communities while
under-allocating scarce tests to higher-burden Latino, Black, and mixed
minority communities, including a zero-allocation path. The right panel shows
context-aware fair routing that uses burden, vulnerability, and site-capacity
context to redirect tests toward higher-risk communities, increasing the
opportunity to prevent missed diagnoses and save lives.}}
...
% \vspace{0.25em}
\caption{\modified{Quantum routing example corresponding to the clinical
routing pattern shown in Figure~\ref{fig:intro_clinical_routing}. The left
panel shows baseline routing that over-allocates attempts to low-demand,
well-calibrated pairs while under-serving high-demand, poorly calibrated pairs.
The right panel shows context-aware iCMAB routing that uses demand,
calibration-quality, and capacity context to reallocate entanglement attempts
toward underserved high-demand flows, improving service opportunity under the
same partial-feedback fairness mechanism.}}
